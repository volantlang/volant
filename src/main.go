...
func main() {
	...
	out, _ := exec.Command("clang", path.Join(path.Dir(file), "_build", "0"+path.Base(file)+".c"), "-pthread", "-luv", "-fblocks", "-lBlocksRuntime", "-lgc", "-lffcall", "-I"+libPath, "-o", path.Join(path.Dir(file), "a.out")).CombinedOutput()
